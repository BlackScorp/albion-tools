...
| T05 Sync-Ablauf | ERLEDIGT | `make verify` erfolgreich; Synchronisierung mit aktuellem Item-Satz, Fehlerstatus, Sperre paralleler Läufe, atomare Speicherung und Rückgabe der frischen API-Beobachtungen |
...
| T08 Stabilisierung | IN ARBEIT | 12.237 IDs, Kategorien, XML-Rezepte, 50er-Paginierung, vollständiger gefilterter Sync und horizontale Stadtauswahl integriert; kurze Itemnamen und technische Kategorie-Dopplungen bereinigt. Mindestgewinn-/ROI-Felder sind verbreitert, der Sync-Button steht unter den Städten. Range-Filter und Sortierung mit fehlenden Werten am Ende ergänzt. Einzelstadt- und einseitige Preisbeobachtungen werden angezeigt; Sync-Status trennt API-Treffer vom lokalen Gesamtbestand. Diagnose-Log schreibt Start, DB-/Beobachtungszahlen, Sync-Ergebnis, Filter-Counts sowie die Item-ID und alle Spaltenwerte jeder sichtbaren Tabellenzeile nach `albion-helper.log` neben die EXE (Fallback: Benutzer-Konfigurationsordner). Die Fyne-Tabelle wird jetzt pro Seite mit einem festen Zeilensnapshot neu aufgebaut, um veraltete Zellinhalte nach Filterwechseln auszuschließen. Rückweg Range 1 → Alle Ranges im UI-Test geprüft. `make test`, `make verify`, `make build-windows` und `git diff --check` erfolgreich. Der gemeldete Laufzeitfehler ist im Test nicht reproduzierbar; Leerlaufmessung der gestarteten Windows-App steht ebenfalls noch aus. |
| T09 Release | OFFEN | – |
...
| 2026-09-16 | T08 Kategorienachbesserung | Die Keulen-Unterkategorien behalten wieder den übergeordneten Knoten „Mace“, während redundante Wiederholungen in den Blättern wegbleiben. Tests decken One Handed und Two Handed ab; `make verify` und `git diff --check` erfolgreich. | Leerlaufmessung auf einer Windows-Desktop-Sitzung nachholen; danach T08 schließen |
| 2026-09-16 | T08 Range-Filter | Range ist als Filter 1–5 verfügbar und nicht mehr sortierbar. Bei Sortierungen bleiben Zeilen ohne Preiswerte unabhängig von der Richtung am Ende; ein positiver Mindestgewinn filtert sie aus. `make verify` und `git diff --check` erfolgreich. | Leerlaufmessung auf einer Windows-Desktop-Sitzung nachholen; danach T08 schließen |
| 2026-09-16 | T08 Preisbeobachtungsanzeige | Einzelstadt- und einseitige Buy-/Sell-Beobachtungen erscheinen direkt in ihren Spalten, ohne Gewinn oder Route zu erfinden. Sync gibt nur frische API-Beobachtungen an die UI zurück und zeigt deren Anzahl getrennt vom gesamten lokalen Bestand. `make verify`, `make test-race`, `make build-windows` und `git diff --check` erfolgreich. | Leerlaufmessung auf einer Windows-Desktop-Sitzung nachholen; danach T08 schließen |
| 2026-09-16 | T08 Range-/Seitenregression | UI-Test durchblättert mit „Alle Ranges“ die Cape-Ergebnisse bis zur Seite mit Preisen und prüft Range, Kauf-/Verkaufspreis sowie Gewinn; der anschließende Range-1-Filter behält das Ergebnis. `make verify` erfolgreich. | Bei erneutem Fehler Laufzeitversion und sichtbare Filterwerte abgleichen |
| 2026-09-16 | T08 Laufzeitdiagnose | Start, lokaler Datenbestand, Sync-Ausgabe und Filterwechsel protokollieren Preis-/Range-Zeilen in `albion-helper.log` neben der EXE; Fallback in den Benutzer-Konfigurationsordner. UI-Regressionsprüfung deckt jetzt auch Range 1 → Alle Ranges ab. `make test`, `make verify`, `make build-windows` und `git diff --check` erfolgreich. Gemeldeter Fehler mit Testdaten nicht reproduziert. | Neue Windows-EXE ausführen, den Range-Wechsel reproduzieren und bei weiterem Fehler `albion-helper.log` prüfen/teilen; anschließend Leerlaufmessung für T08 |
| 2026-09-16 | T08 Tabellenzeilen-Diagnose | Log erweitert: bei jeder Tabellenaktualisierung stehen aktuelle Seite, Filter-Range, Item-ID und die Spaltenwerte jeder sichtbaren Zeile in Tabellenreihenfolge im Log. `make verify`, `make build-windows` und `git diff --check` erfolgreich. | Mit neuer EXE Range 1 und Alle Ranges vergleichen; die `Tabellenzeile`-Einträge samt jeweiliger `FilterRange` zurückmelden |
| 2026-09-16 | T08 Zellaktualisierung | Die Tabellenzeilen werden pro Seitenwechsel/Filterwechsel aus einem unveränderlichen Zeilensnapshot neu aufgebaut. UI-Test prüft Preise/Range vor, während und nach Range 1 → Alle Ranges mit der jeweils aktuellen Tabelleninstanz. `make verify`, `make build-windows` und `git diff --check` erfolgreich. | Neue Windows-EXE testen; falls Anzeige weiterhin von den geloggten `Werte` abweicht, Fenster-/DPI-Details und neues Log vergleichen |
